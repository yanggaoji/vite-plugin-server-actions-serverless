...
export function enhanceFunctionNotFoundError(functionName, moduleName, availableFunctions = []) {
  ...
  const similarFunctions = availableFunctions.filter(fn => 
    levenshteinDistance(fn, functionName) <= 2
  );
...
export function enhanceValidationError(validationErrors, functionName) {
  ...
  const hasTypeErrors = validationErrors.some(err => 
    err.message.includes('Expected') || err.message.includes('Invalid')
  );
  ...
  const hasRequiredErrors = validationErrors.some(err => 
    err.message.includes('required') || err.message.includes('missing')
  );
  ...
  const hasFormatErrors = validationErrors.some(err => 
    err.message.includes('format') || err.message.includes('pattern')
  );
