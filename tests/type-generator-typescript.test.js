...
describe("Type Generator - TypeScript Support", () => {
  describe("Type Definition Generation", () => {
    ...
    it("should handle complex parameter types", () => {
      ...
      expect(typeDefs).toContain("function complexFunc(user: User, options?: { includeDeleted?: boolean; limit?: number }, ...tags: string[]): Promise<Result<User[]>>");
    });
 ...
 */`
    ...
    it("should handle destructured parameters", () => {
      ...
      expect(typeDefs).toContain("function processData({id, name}: { id: number; name: string }, [first, second]: [string, number]): Promise<void>");
    });
  });
          ...
          functionDetails: [
            {
              ...
              isAsync: false,
              params: [
                { name: "value", type: "T extends string ? string[] : number[]" }
              ],
              returnType: "T",
              jsdoc: null
            }
