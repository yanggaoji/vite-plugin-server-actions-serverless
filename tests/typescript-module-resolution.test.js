...
describe("TypeScript Module Resolution Fix", () => {
  ...
  it("should use ssrLoadModule when Vite server is available", async () => {
    ...
    // Load the module
    await plugin.load(testPath);

    // Verify ssrLoadModule was called
    expect(mockViteServer.ssrLoadModule).toHaveBeenCalledWith(testPath);
  });
